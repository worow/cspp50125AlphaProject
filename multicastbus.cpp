...
int main(int argc, char** argv){
	...
	struct TradeMessage *tmsg;
    tmsg = (struct TradeMessage*) malloc(sizeof(struct TradeMessage));
	...
	while(1){
		...
		memset(tmsg, 0, BUFSIZE);
		memset(&sender_addr,0,sizeof(struct sockaddr_in));
		if(recvfrom(sockfd,tmsg,BUFSIZE, 0,(struct sockaddr*) &sender_addr,(socklen_t*) &frmlen) < 0){
			perror("recvfrom");
			exit(EXIT_FAILURE);
		}
		
		printf("Trade: %s %s %ld\n", tmsg->symbol,tmsg->price,tmsg->quantity);
